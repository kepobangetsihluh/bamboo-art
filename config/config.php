...
function register($data)
{
    ...
    $query = "INSERT INTO users VALUES ('', '$nama', '$email', '$password', null, '$number', '', 'USER')";
...
// Add To cart
// FIXED BY MUHAMMAD FIKRI ARZYAH (8/11/2022)
// DON'T DELETE THIS OR YOU CANT CLAIM GUARANTEE
function addToCart($data) 
{
    ...
    $queryCart      = "INSERT INTO carts VALUES ('', '$user_id', '$product_id', '$banyak', '$total')";
    
    mysqli_query($conn, $queryCart);

    $stockProduk    =  $product["stock"] - ($banyak * 1);
    $queryProduct   = "UPDATE products SET stock = '$stockProduk' WHERE id_product = $product_id ";
...
function deleteProductAtCart($data)
{
    ...
    $stokProduct = $data["stokProduct"];
    $product_id = $data["idProduct"]; 

    mysqli_query($conn, "DELETE FROM carts WHERE id_cart = $idProduct");
    mysqli_query($conn, "UPDATE products SET stock = stock+$stokProduct WHERE id_product = $product_id");
